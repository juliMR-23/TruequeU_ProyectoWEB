...
import { FiHome, FiList, FiStar, FiUser, FiMenu, FiX, FiMessageSquare, FiShield } from "react-icons/fi";
        ...
        <header className="w-full bg-eia-azul shadow-md sticky top-0 z-50">
            <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-3">
                ...
                `} aria-label="Nav principal">
                    ...
                    {/* Siempre visibles — rutas públicas */}
                    <NavLink to="/" onClick={closeMenu}
                        className={({ isActive }) => isActive ? `${linkBase} ${activeClass}` : linkBase}>
                        <FiHome /> <span>Home</span>
                    </NavLink>

                    <NavLink to="/publicaciones" onClick={closeMenu}
                        className={({ isActive }) => isActive ? `${linkBase} ${activeClass}` : linkBase}>
                    ...
                    {user?.role === "Admin" && (
                        <NavLink to="/admin" onClick={closeMenu}
                            className={({ isActive }) => isActive ? `${linkBase} ${activeClass}` : linkBase}>
                            <FiShield /> <span>Panel Admin</span>
                        </NavLink>
                    )}
                    ...
                    {user?.role === "Client" && (
                        <>
                            <NavLink to="/chats" onClick={closeMenu}
                                className={({ isActive }) => isActive ? `${linkBase} ${activeClass}` : linkBase}>
                                <FiMessageSquare /> <span>Chats</span>
                            </NavLink>

                            <NavLink to="/favoritos" onClick={closeMenu}
                                className={({ isActive }) => isActive ? `${linkBase} ${activeClass}` : linkBase}>
                    ...
                    {!user && (
                        <NavLink to="/login" onClick={closeMenu}
                            className={({ isActive }) => isActive ? `${linkBase} ${activeClass}` : linkBase}>
                            <FiUser /> <span>Iniciar sesión</span>
                        </NavLink>
                    )}
                </nav>
            </div>
